...
#define GBUFFER_SLOT_NUM_SLOTS 3
...
float4 PSMain(VSOutput input) : SV_Target0
{
    ...
    float sceneDepth = SceneDepth.SampleLevel(StaticSampler, input.uv, 0.0).r;
    float3 worldPos = WorldPosFromDepth(viewProjInv, input.uv, sceneDepth);
    float3 eye = cbViewData.viewInv[3].xyz;
    
    float3 albedo = GbufferTextures[GBUFFER_SLOT_ALBEDO].Sample(StaticSampler, input.uv).rgb;
    float2 roughnessMetalness = GbufferTextures[GBUFFER_SLOT_ROUGHNESS_METALNESS].Sample(StaticSampler, input.uv).rg;
    
    // As of 16.02.2024 -> .xy - geometry normal packed, .zw - surface normal packed
    float4 normals = GbufferTextures[GBUFFER_SLOT_NORMAL].Sample(StaticSampler, input.uv);
